...
OUT_SVG = os.path.join(OUT_DIR, "contract_cycles_vs_workset_d8_l1l2.svg")
...
def main():
    ...
    for r in rows:
        if DEPTH_FILTER and r.get("depth") != str(DEPTH_FILTER):
            continue
        workset = r["workset"]
        if workset not in WORKSET_ORDER:
            continue
        opt = r["opt"]
        mode = r["mode"]
    ...
    if values:
        ...
        min_y, max_y = 0.0, 1.0
    pad = (max_y - min_y) * 0.2 if max_y > min_y else 1.0
    ...
    lines.append(f"<text x='{W/2}' y='20' text-anchor='middle' font-family='sans-serif' font-size='16'>Cycles/Iter (Zoom L1/L2, {title_depth})</text>")
    ...
    for i in range(5):
        ...
        lines.append(f"<line x1='{x0}' y1='{py}' x2='{x1}' y2='{py}' stroke='#eee'/>")
