...
def write_outputs(results: Dict[str, Dict[str, int]], out_dir: str):
    rows = []
    ...
    csv_path = os.path.join(out_dir, "results_hotspot.csv")
    ...
    md_path = os.path.join(out_dir, "results_hotspot.md")
...
def main() -> None:
    out_dir = os.path.join("build", "hotspot")
    java_src = os.path.join("hotspot", "ChaseBench.java")
    # WSL can have permissions issues on /mnt/c; compile from build/hotspot copy.
    java_copy = os.path.join(out_dir, "ChaseBench.java")
    if not os.path.exists(java_copy):
        os.makedirs(out_dir, exist_ok=True)
        with open(java_src, "r", encoding="utf-8") as src:
            with open(java_copy, "w", newline="\n") as dst:
                dst.write(src.read())
    compile_java(java_copy, out_dir)
    log_path = run_java(out_dir, out_dir)
