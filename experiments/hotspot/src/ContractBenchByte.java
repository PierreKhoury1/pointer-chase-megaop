public class ContractBenchByte {
  static volatile int sink;

  static byte[] buildRing(int nodes) {
    if (nodes > 256) {
      System.err.println("byte index graph supports up to 256 nodes");
      System.exit(1);
    }
    byte[] next = new byte[nodes];
  ...
  static int chaseGuarded(byte[] next, int idx, int depth) {
    int n = next.length;
  ...
  static int chaseContract(byte[] next, int idx, int depth) {
    for (int i = 0; i < depth; i++) {
      idx = next[idx] & 0xFF;
    }
    return idx;
  ...
  static int runGuarded(byte[] next, int depth, int iters) {
    int acc = 0;
    int idx = 0;
  ...
  static int runContract(byte[] next, int depth, int iters) {
    int acc = 0;
    int idx = 0;
    int stride = 1315423911 % next.length;
    if (stride == 0) {
  ...
  static void warmup(byte[] next, int depth, int iters) {
    int acc = 0;
    for (int i = 0; i < 3; i++) {
  ...
  public static void main(String[] args) {
    if (args.length < 4) {
      System.err.println("usage: ContractBenchByte <mode> <depth> <nodes> <iters>");
