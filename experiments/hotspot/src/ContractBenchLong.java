public class ContractBenchLong {
  static volatile long sink;

  static long[] buildRing(int nodes) {
    long[] next = new long[nodes];
  ...
  static long chaseGuarded(long[] next, int idx, int depth) {
    int n = next.length;
  ...
  static long chaseContract(long[] next, int idx, int depth) {
    for (int i = 0; i < depth; i++) {
      idx = (int) next[idx];
    }
    return idx;
  ...
  static long runGuarded(long[] next, int depth, int iters) {
    long acc = 0;
    int idx = 0;
  ...
  static long runContract(long[] next, int depth, int iters) {
    long acc = 0;
    int idx = 0;
    int stride = 1315423911 % next.length;
    if (stride == 0) {
  ...
  static void warmup(long[] next, int depth, int iters) {
    long acc = 0;
    for (int i = 0; i < 3; i++) {
  ...
  public static void main(String[] args) {
    if (args.length < 4) {
      System.err.println("usage: ContractBenchLong <mode> <depth> <nodes> <iters>");
