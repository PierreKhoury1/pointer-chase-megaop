...
SRC_DIR = EXP_DIR / "src"
CLASS_DIR = EXP_DIR / ".build" / "classes_short"
...
def build_worksets() -> list[tuple[str, int, int, int]]:
    ...
    if "L3" in sizes:
        ...
        nodes = min(nodes, MAX_NODES)
        worksets.append(("L3", nodes * elem_size, nodes, 1_000_000))
    return worksets
...
def compile_java():
    ...
    subprocess.run(
        ["javac", "-d", str(CLASS_DIR), str(SRC_DIR / "ContractBenchShort.java")],
        check=True,
    )
...
def main():
    ...
    out_csv = RAW_DIR / "results_contract_jit_short_perf.csv"
    out_md = PROC_DIR / "results_contract_jit_short_perf.md"
    ...
    with open(out_md, "w", newline="\n") as f:
        f.write("## Pointer-chase contract (JIT short) perf (depth=8, guarded)\n\n")
        f.write(f"- perf binary: `{perf_path}`\n")
        f.write("- taskset: `-c 0`\n")
        f.write("- JVM: `-Xms512m -Xmx512m -XX:+UseSerialGC -XX:-TieredCompilation`\n")
        f.write("- NOTE: short index graph is capped at 65535 nodes.\n\n")
        header = (
            "| workset | bytes | nodes | iters | mode | depth | "
            + " | ".join(EVENTS)
            + " | cycles/iter |\n"
