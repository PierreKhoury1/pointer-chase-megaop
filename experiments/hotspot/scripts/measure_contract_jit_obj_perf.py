...
SRC_DIR = EXP_DIR / "src"
CLASS_DIR = EXP_DIR / ".build" / "classes_obj"
RAW_DIR = EXP_DIR / "data" / "raw"
PROC_DIR = EXP_DIR / "data" / "processed"
PLOT_DIR = EXP_DIR / "plots" / "jit_obj"
...
def build_worksets() -> list[tuple[str, int, int, int]]:
    ...
    node_size = 24  # approximate object size (header + ref + int + padding)
    worksets = []
    if "L1" in sizes:
        nodes = max(512, sizes["L1"] // node_size)
        worksets.append(("L1", sizes["L1"], nodes, 5_000_000))
    if "L2" in sizes:
        nodes = max(2048, sizes["L2"] // node_size)
        worksets.append(("L2", sizes["L2"], nodes, 3_000_000))
    if "L3" in sizes:
        nodes = max(16384, sizes["L3"] // node_size)
        worksets.append(("L3", sizes["L3"], nodes, 1_000_000))
    dram_bytes = 64 * 1024 * 1024
    nodes = max(65536, dram_bytes // node_size)
...
def run_perf(perf_path: str, mode: str, depth: int, nodes: int, iters: int) -> dict:
    cmd = [
        ...
        "-XX:CompileCommand=dontinline,ContractBenchObj::chaseGuarded",
        "-XX:CompileCommand=dontinline,ContractBenchObj::chaseContract",
        "ContractBenchObj",
        mode,
        str(depth),
        str(nodes),
...
def compile_java():
    ...
    subprocess.run(
        ["javac", "-d", str(CLASS_DIR), str(SRC_DIR / "ContractBenchObj.java")],
        check=True,
    )
...
def main():
    ...
    out_csv = RAW_DIR / "results_contract_jit_obj_perf.csv"
    out_md = PROC_DIR / "results_contract_jit_obj_perf.md"
    ...
    with open(out_md, "w", newline="\n") as f:
        f.write("## Pointer-chase contract (JIT object) perf (per depth)\n\n")
        f.write(f"- perf binary: `{perf_path}`\n")
        f.write("- taskset: `-c 0`\n")
        f.write("- JVM: `-Xms512m -Xmx512m -XX:+UseSerialGC -XX:-TieredCompilation`\n")
        f.write("- CompileCommand: `dontinline` for chaseGuarded/chaseContract\n\n")
        header = (
            "| workset | bytes | nodes | iters | mode | depth | "
            + " | ".join(EVENTS)
            + " | cycles/iter |\n"
    ...
    graph_dir = PLOT_DIR
    out_svg = graph_dir / "contract_jit_obj_speedup_vs_workset.svg"
    ...
    lines.append(f"<text x='{W/2}' y='20' text-anchor='middle' font-family='sans-serif' font-size='16'>JIT Object Speedup vs Working Set Size</text>")
    ...
    if converter:
        out_png = graph_dir / "contract_jit_obj_speedup_vs_workset.png"
