...
SRC_DIR = EXP_DIR / "src"
CLASS_DIR = EXP_DIR / ".build" / "classes_long"
...
def compile_java():
    ...
    subprocess.run(
        ["javac", "-d", str(CLASS_DIR), str(SRC_DIR / "ContractBenchLong.java")],
        check=True,
    )
...
def main():
    ...
    out_csv = RAW_DIR / "results_contract_jit_long_perf.csv"
    out_md = PROC_DIR / "results_contract_jit_long_perf.md"
    ...
    with open(out_md, "w", newline="\n") as f:
        f.write("## Pointer-chase contract (JIT long) perf (depth=8, guarded)\n\n")
