...
ASM_DIR = AOT_DIR / "asm"
GEN_DIR = AOT_DIR / "generated"
RAW_DIR = AOT_DIR / "data" / "raw"
PROC_DIR = AOT_DIR / "data" / "processed"
...
def generate_c(path: Path) -> List[str]:
    lines: List[str] = []
    ...
    for depth in DEPTHS:
        ...
        offsets = [str(const_offsets[i % len(const_offsets)]) for i in range(depth)]
...
def compile_and_count(c_path: Path, func_names: List[str], compiler: str, opt: str) -> Dict[str, Counts]:
    asm_path = ASM_DIR / f"{compiler}_O{opt}.s"
    cmd = [
        compiler,
        "-S",
        f"-O{opt}",
...
def main() -> None:
    ...
    c_path = GEN_DIR / "generated_pointer_chase.c"
    ...
    # Write CSV
    csv_path = RAW_DIR / "results_aot.csv"
    ...
    md_path = PROC_DIR / "results_aot.md"
