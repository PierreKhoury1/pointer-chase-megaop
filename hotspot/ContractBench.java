public class ContractBench {
  ...
  static int[] buildRing(int nodes) {
  ...
  static int chaseGuarded(int[] next, int idx, int depth) {
    int n = next.length;
  ...
  static int chaseContract(int[] next, int idx, int depth) {
    for (int i = 0; i < depth; i++) {
      idx = next[idx];
    }
    return idx;
  ...
  static int runGuarded(int[] next, int depth, int iters) {
    int acc = 0;
    int idx = 0;
  ...
  static int runContract(int[] next, int depth, int iters) {
    int acc = 0;
    int idx = 0;
    int stride = 1315423911 % next.length;
    if (stride == 0) {
  ...
  static void warmup(int[] next, int depth, int iters) {
    int acc = 0;
    for (int i = 0; i < 3; i++) {
  ...
  public static void main(String[] args) {
    if (args.length < 4) {
      System.err.println("usage: ContractBench <mode> <depth> <nodes> <iters>");
